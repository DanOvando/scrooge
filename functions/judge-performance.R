judge_performance <- function(observed, predicted, observed_variable = f, predicted_variable = "f_t", group_level = year){
  ...
  comparison_plot <- comparison %>%
    ...
    labs(caption = glue::glue("RMSE = {prettyNum(comparison_summary$rmse, digits = 2)} ; Bias = {prettyNum(comparison_summary$bias, digits = 2)}"))
  ...
  # comparison_plot + gridExtra::grid.table(comparison_summary)
