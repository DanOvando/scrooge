---
title: "Bringing Economic Knowledge into Fisheries Stock Assessment"
...
- Litte data and lots of needs creates demand for DLAs
...
A few plots of open access dynamics in theory and practice
...
```{r}
(r_lengths + labs(x = "")) + (f_lengths + labs(title = '')) + plot_layout(ncol = 2)
```
...
Lime: $f_{t} \sim normal(f_{t-1},\sigma_{f})$

---

#Scrooge

Two paths for economic data (always $f_{t} = q_{t}E_{t}$)

- Effort history ("Effort is about twice what it used to be")

$$\Delta_{t}^{effort} = \frac{effort_{t+1}}{effort_{t}}$$

$$\hat{f_{t}}^{effort} = f_{t-1}\Delta_{t-1}$$
  
- Incentives history ("prices went up a lot the last few years")

$$p_t = p_{base}\frac{p^{obs}_t}{\bar{p^{obs}}}$$

$$\Pi_{t-1} = p_{t}q_{t}E_{t}B^{c}_{t} - c_{t}E_{t}^{\beta}$$

$$\hat{f_{t}}^{econ} = f_{t-1} + f_{msy}(\theta\frac{\Pi_{t-1}}{\Pi_{msy}})$$

- Expected effort is then...

$$f_t \sim normal(\alpha\hat{f_{t}}^{econ} + (1 - \alpha)\hat{f_{t}}^{effort},\sigma_{f})$$
...
- Proposed solution: *Predictive algorithm for model performance*
  ...
  - Calculate metric of interst (RMSE, bias, etc.)
