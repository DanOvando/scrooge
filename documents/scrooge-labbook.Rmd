...
test_catch <- array_branch(check$c_ta,1)
...
test_length %>% 
  slice(1) %>% 
  gather(length, numbers) %>% 
  mutate(true_numbers = b) %>% 
  mutate(length = str_replace_all(length,"\\D","") %>% as.numeric()) %>% 
  gather(source, numbers,-length)  %>% 
  group_by(source) %>% 
  mutate(snumbers = numbers / sum(numbers)) %>% 
  ggplot(aes(length, snumbers, fill = source)) + 
