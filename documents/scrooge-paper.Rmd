...
We propose a similar use for profit per unit effort, but rather than PPUE informing an index of abundance, it serves as an index in the rate of change in effort (and by extension conditaionl on *q*), fishing mortality. Per a standard open-access model of effort development, 
...
We can therefore use PPUE as an index of the change in timestep-to-timestep effort. In other words, given that our model estiamtes $\hat{E}$ through one of our process models, 
...
#### Index of Effort Changes (`length + effort`)
...
Limitations
- what about a managed fishery?
- importance of priors to estimate all this crap

garbage in garbage out - but same problem for any data and you can talk to humans. 
