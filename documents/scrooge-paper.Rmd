...
All effort process model have an expected process and a process error component. The process error, $E^{dev}$, is estiamted in the same manner across all the process models 
...
where $\hat{PPUE_{t}}$ the profit per unit effort, and $\theta$ adjusts the responsiveness of effort to *PPUE*. Note that the model includes the same effort process error as utilzied in the random-walk processmodel. 
...
Lastly, we also used LIME to estimate fishing mortality for each of the `r nrow(processed_grid)` simulated assessments scenarios, and then for each scenario calcualted the difference in rmse resulting from LIME and `scrooge`
...
sandbox_performance <- processed_sandbox %>%
  ...
  select(-scrooge_fit, -prepped_fishery,-summary_plot,-fleet_params,-lime_v_scrooge) %>%
  unnest()
