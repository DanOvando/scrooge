---
title: "Improving Fisheries Stock Assessment by Integrating Economic Data"
author: "Dan Ovando"
date: "3/7/2018"
output: 
...
This estimation model is similar in flavor to the penalty on deviations in fishing mortality used in LIME. 
...
where *C()* represents that Baranov catch equation [@Baranov1918]. 
...
where $\hat{PPUE_{t}}$ the profit per unit effort, and $\theta$ adjusts the responsiveness of effort to *PPUE*. Note that the model includes the same effort process error as utilized in the random-walk process model. 
...
In a similar vein, comparisons between `scrooge` and other assessments such as LIME as in no way intended to establish a "winner". Our results show that, under the very specific set of circumstances simulated here, inclusion of economic theory and data through `scrooge` often outperformed, in root median squared error, estimation by `LIME` using only length-composition data in the likelihood. However, we also found many cases where `LIME` outperformed `scrooge` (Fig.\@ref(fig:scrooge-v-lime)). Similar to the decision tree method we used to for model selection within `scrooge`, we can also start using this simulation testing method to develop decision tree across models. Two areas of particularly interesting research will be a) how flawed economic data and/or assumptions can be before its inclusion becomes useless or actively harmful and b) comparison of lengths + economic data to for example lengths + CPUE, or lengths + CPUE + PPUE. 
...
```{r lcomp-pp, fig.cap = "90% pospterior predictive distribution (shaded ribbon) ot fitted length composition data (points). Nearly observed points fall within the posterior predictive distribution", eval = TRUE}
...
```{r mean-length, fig.cap="Posterior predictive distribution of mean length (distribution) and observed mean length (red line) over time. True mean lengths all fell within posterior predictive distribution", eval = TRUE}
