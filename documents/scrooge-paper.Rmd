...
- @Marchal2013 added value of fleet dynamics in models. This one looks at MSE style problems again, but has some interesting fleet model specifications that you could steal. Equation 11 is handy, just more justification for change is the effort last year times some factor and profits over the last few years
...
## Methods
  ...
  - Fishing mortality rates from the simulated fishery are estimated using our bio-economic estimation model, which can take a variety of forms
...
modelo_name <- c(
  em0_lm0 = "Lengths Only (`random walk` + `lcomps`)",
  em1_lm1 =  "Lengths + Economics (`ingredients` + `lcomps + ppue`)"
)
...
Running comparison to LIME and LBSPR now
